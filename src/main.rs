        ...
        if event::poll(Duration::from_millis(100))? {
            if let Event::Key(key) = event::read()? {
                ...
                match key.code {
                    ...
                    KeyCode::Char('j') | KeyCode::Down if app.scroll < app.max_scroll() => {
                        app.scroll += 1;
                    }
...
fn draw_boot(f: &mut ratatui::Frame, area: Rect, app: &App) {
    ...
    let rows: Vec<Row> = app.blame[start..end]
        .iter()
        .map(|b| {
            let color = if b.time_ms > 5000 {
                RED
            } else if b.time_ms > 1000 {
                YELLOW
            } else {
                GREEN
            };
    ...
    let blame_table = Table::new(rows, [Constraint::Length(15), Constraint::Min(30)])
        .header(header)
        .block(
            Block::default()
                .borders(Borders::ALL)
...
fn draw_journal(f: &mut ratatui::Frame, area: Rect, app: &App) {
    ...
    let rows: Vec<Row> = app.journal[start..end]
        .iter()
        .map(|j| {
